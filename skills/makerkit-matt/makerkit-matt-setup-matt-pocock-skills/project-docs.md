...
If the concept you need isn't named anywhere in the `AGENTS.md` set, that's a signal: either you're inventing language the project doesn't use (reconsider) or there's a real gap (note it for `/makerkit-matt-domain-modeling`, which records new terms into the owning `AGENTS.md`).
