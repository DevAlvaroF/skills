...
**Support tickets stay in Postgres rather than an external helpdesk.**
We evaluated Zendesk and Intercom; both would put ticket data outside RLS, and tenant isolation is the property we most need to prove. Keeping tickets in `public.tickets` costs us the helpdesk UI but keeps every read behind a policy.
```
