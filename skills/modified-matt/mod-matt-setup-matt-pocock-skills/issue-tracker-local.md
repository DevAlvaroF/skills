...
Specs for this repo live as markdown files in `.myprds/`; **tickets are JSON files**, so other software can read them without parsing prose.
...
Create a new file under `.myprds/<NN>-<feature-slug>/` (creating the directory if needed): a `.json` ticket under `issues/`, or a markdown file for a spec.
