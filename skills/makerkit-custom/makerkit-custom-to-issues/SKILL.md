...
The issue tracker conventions should have been provided to you. If not, tell the user to run `/makerkit-custom-setup-skills`.
...
If you have not already explored the codebase, do so to understand the current state of the code. Read the root `AGENTS.md`, then the nearest `AGENTS.md` to the code involved, and follow the doc-consumption and vocabulary rules they set out, so issue titles and descriptions use the project's own terms. Where those files and this description differ, **follow the repo**.
