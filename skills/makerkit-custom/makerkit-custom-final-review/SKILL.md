...
5. **Check.** Run the repo's verification steps from the root `AGENTS.md` § Verification, in the order given there,
   sending any whole-repo run (full suite, build, lint sweep) through a sub-agent that reports failures only. Then
   review your own changes against the findings.
6. **Close or report.**
